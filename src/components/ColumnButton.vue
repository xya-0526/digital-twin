    ...
    <div class="Butbox">
        ...
        <div class="Butbox1">
            <div class="Butbox2" style="width: 14.5rem; height: 4.7rem;" v-for="(button, index) in buttons">
                <span class="T"></span>
                <UniversalButton :margin="'0 2.6rem 0 0'" :justifyContent="'end'" :Font_size="'2.8rem'" :width="'100%'"
                    :height="'100%'" :Border="'0.1rem solid #3c5446'" :border_radius="'4rem'" :key="index"
                    :text="button.text" :image="button.image"
                    :bgColor="activeButton == index ? 'linear-gradient(90deg, #1B3C2E 0%, #44A562 100%)' : '#202C2A'"
                    :color="activeButton == index ? '#fff' : '#FFFFFF'" @click="addActive(index)">
                </UniversalButton>
            </div>
        </div>
    </div>
